...
function GaugeCard({ kpi, delay }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -3 }}
      className="rounded-xl border border-brand-100 bg-white p-3 shadow-sm hover:shadow-md transition-shadow flex flex-col items-center"
    >
      <div className="w-full h-24 relative">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <p className="text-xl font-bold text-brand-900">{kpi.value}%</p>
          <p className={`text-[10px] mt-0.5 flex items-center gap-0.5 ${above ? "text-emerald-600" : "text-amber-600"}`}>
            {above ? <TrendingUp size={10} /> : <Minus size={10} />}
            {above ? "+" : "-"}{gap}% vs KH
          </p>
        </div>
      </div>
      <p className="mt-1 text-xs font-semibold text-brand-900 text-center leading-tight">{kpi.label}</p>
    </motion.div>
  );
}
...
function MiniTrendChart() {
  return (
    <div className="h-full flex flex-col">
      <p className="text-xs font-bold text-slate-400 uppercase tracking-wide mb-2 shrink-0">
        Diễn biến DT – CP – LN theo tháng (tỷ đồng)
      </p>
...
function KpiRow({ item, delay }) {
  ...
  return (
    <motion.div
      ...
      className="flex items-center gap-2.5 py-1 border-b border-slate-100 last:border-0"
    >
      {/* Status dot */}
      <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: s.dot }} />

      {/* Label */}
      <p className="text-xs text-slate-700 w-32 shrink-0 truncate">{item.label}</p>

      {/* Value + unit */}
      <p className="text-xs font-bold text-brand-900 w-14 text-right shrink-0">
        {item.th5T}
      ...
      {/* %KH */}
      <p className="text-xs font-semibold w-9 text-right shrink-0" style={{ color: s.text }}>
        {item.khPct}%
      </p>

      {/* Status badge (compact) */}
      <span
        className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full w-20 text-center shrink-0"
  ...
  return (
    <SlideShell index={index} total={total}>
      <div className="flex-1 flex flex-col max-w-5xl mx-auto w-full">
        ...
        {/* ── 4 KPI Gauges ── */}
        <div className="mt-3 grid grid-cols-4 gap-3 shrink-0">
        ...
        <div className="mt-3 grid grid-cols-5 gap-4 min-h-0" style={{ height: "11rem" }}>
          {/* Chart */}
          <motion.div
            ...
            className="col-span-3 rounded-xl border border-brand-100 bg-white p-4 shadow-sm"
          >
            <MiniTrendChart />
          </motion.div>
          ...
          >
            ...
            <div className="absolute bottom-4 left-4 right-4 text-white">
              ...
              <p className="text-[11px] text-brand-100 mt-1 leading-relaxed">
        ...
        <motion.div
          ...
          className="mt-3 rounded-xl border border-brand-100 bg-white p-4 shadow-sm"
        >
          ...
          <div className="mt-2 pt-2 border-t border-brand-100 flex items-center gap-2 text-[11px] text-slate-500">
